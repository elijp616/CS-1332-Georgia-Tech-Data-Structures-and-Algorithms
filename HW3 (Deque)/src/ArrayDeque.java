...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            throw new IllegalArgumentException("You cannot insert null data");
        } else {
            if (size == backingArray.length && size != 0) {
                T[] temporary = (T[]) new Object[this.backingArray.length * 2];
                for (int i = 0; i < backingArray.length; i++) {
                    temporary[i + 1] =
                            this.backingArray[mod(front, backingArray.length)];
                    front++;
                }
                backingArray = temporary;
                front = 0;
    ...
    public void addLast(T data) {
        if (data == null) {
            ...
            if (size == backingArray.length && size != 0) {
                T[] temporary = (T[])
                        new Object[this.backingArray.length * 2];
                for (int i = 0; i < backingArray.length; i++) {
                    temporary[i] =
                            backingArray[mod(front, backingArray.length)];
                    front += 1;
                }
                backingArray = temporary;
                front = 0;
                this.backingArray[mod(front + size, backingArray.length)]
                        = data;
                size += 1;
            } else {
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("There is nothing in the deque");
        }
    ...
    public T getFirst() {
        if (size == 0) {
            throw new NoSuchElementException("There is "
                   + "nothing in the deque to retrieve");
        }
